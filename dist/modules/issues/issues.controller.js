...
const getAllIssues = expressAsyncHandler(async (req, res) => {
...
const getSingleIssue = expressAsyncHandler(async (req, res) => {
    const { id } = req.params;
    const data = await issuesService.getSingleIssueFromDB(id);
    if (!data) {
        res.status(404);
...
const updateIssue = expressAsyncHandler(async (req, res) => {
    const { id } = req.params;
    ...
    const result = await issuesService.updateIssueInDB(id, currentUser, req.body);
